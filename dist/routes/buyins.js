...
router.post('/', auth_1.protect, async (req, res) => {
    try {
        ...
        const { campusId: bodyCampusId, date: dateStr, amount } = req.body;
        // Determine campus
        const campusId = bodyCampusId || req.user?.campusId;
        if (!campusId)
            return res.status(400).json({ error: 'campusId required' });
        let date;
        if (dateStr) {
        ...
        const created = await app_1.prisma.buyIn.upsert({
...
router.get('/status', auth_1.protect, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        const campusId = req.query.campusId || req.user?.campusId;
        if (!campusId)
            return res.status(400).json({ error: 'campusId required' });
        ...
        const found = await app_1.prisma.buyIn.findUnique({ where: { userId_campusId_date: { userId, campusId, date } } });
